...
public class Notice_DAO {
	...
	public ArrayList<Notice_DTO> getNoticeIndex(){ //index notice list
		ArrayList<Notice_DTO> dtos = new ArrayList<Notice_DTO>();
		String query = " select notice_no, substr(title,1,10), substr(content,1,5), to_char(reg_date, 'yyyy-mm-dd') "+
					   " from (select notice_no, title, content, reg_date from A02_TRACK2_WEB_NOTICE "+
		...
		try {
			...
			System.out.println("Exception getNoticeIndex():"+e.getMessage());
		} finally {
			try {
				common.close(con,ps,rs);
			}catch(Exception e) {
				System.out.println("getNoticeIndex() close:"+e.getMessage());
			}
		}
		return dtos;
	}	
	...
	public int noticeHit(String noti_no){ //조회수 증가
		int result = 0;
		String query = " update A02_TRACK2_WEB_NOTICE set hit = hit+1 where notice_no ='"+noti_no+"' ";
		try {
			con = common.getConnection();
			ps  = con.prepareStatement(query);
	...
	public Notice_DTO getNoticeView(String noti_no){ //상세조회
		...
		String query = " select a.notice_no, a.title, a.content, a.file_name_1, b.name, to_char(a.reg_date, 'yy-MM-dd'), a.hit "+
					   " from A02_TRACK2_WEB_NOTICE a, A02_TRACK2_WEB_MEMBER b "+
					   " where a.reg_id = b.id "+
	...
	public int deleteNotice(String notice_no) { //공지사항 삭제
		int result = 0;	
		String query = " delete from A02_TRACK2_WEB_NOTICE where notice_no = '"+notice_no+"' ";
		try {
			con = common.getConnection();
			ps  = con.prepareStatement(query);
			result  = ps.executeUpdate();
	...
	public int updateNotice(String notice_no, String title, String content, String file_name_1, String reg_id, String reg_date) { //공지사항 수정
		int result = 0;	
		String query = " update A02_TRACK2_WEB_NOTICE set title='"+title+"', content='"+content+"', file_name_1='"+file_name_1+"', reg_id='"+reg_id+"', reg_date = '"+reg_date+"' "+
	...
	public int insertNotice(Notice_DTO dto) { //공지사항 등록
		int result = 0;	
		String query = " insert into A02_track2_web_notice(notice_no, title, content, file_name_1, reg_id, reg_date, hit) "+
	...
	public int insertNotice(String notice_no, String title, String content, String reg_id, String reg_date) { //공지사항 등록
		int result = 0;	
		String query = " insert into a02_track2_web_notice (notice_no, title, content, reg_id, reg_date) "+ 
	...
	public String getMaxNo(){  //notice_no 최대값 검사
		String query  = " select max(notice_no) from A02_TRACK2_WEB_NOTICE ";
		String result = null;
		try {
			con = common.getConnection();
			ps  = con.prepareStatement(query);
