...
<%
	...
	if(selValue == null){
		...
		ArrayList<Qanda_DTO> dtos = dao.getQandaList(selValue,txtValue);
	...
	function formSearch(){ //제목 검색
		...
		<p class="select_Box">
			<select name="t_sel">
				...
				<option value="question" <%if(selValue.equals("question")) out.print("selected");%>>내용</option>
			</select>
...
%> 	
			<tr>									
				<td>
<%
				if(dtos.get(k).getSecret().equals("[공개]") || sessionId.equals("manager")||sessionId.equals(dtos.get(k).getUser_id())){
%>
				<a href="qanda_v.jsp?t_qandaNo=<%=dtos.get(k).getQanda_no()%>">
<%
				}				
%>						
				<%=dtos.get(k).getQanda_no()%></td>
				<td class="title">
<%
				if( dtos.get(k).getSecret().equals("[공개]") || sessionId.equals("manager")||sessionId.equals(dtos.get(k).getUser_id())){
%>
...
<%
				if( dtos.get(k).getSecret().equals("[비공개]")){
%>	
				<%=dtos.get(k).getTitle()%>&nbsp;&nbsp;&nbsp;<i class="fas fa-lock"></i></a>
<%
				}else{				
%>			
				<%=dtos.get(k).getTitle()%>
...
%>
						...
						<td><%=dtos.get(k).getUser_name()%></td>					
						<td><%=dtos.get(k).getReg_date_q()%></td>
...
<%	} %> 					
			...
			<div class="paging">
<%
			url = "qanda_r.jsp?t_sel="+selValue+"&t_search="+txtValue;		
			out.println(CommonUtil.pageList(current_page, total_page, url));
%>	
				<a href="/qanda/qanda_w.jsp" class="btn_write">글쓰기</a> 
			</div>
		</div>
	</div>
<style>
