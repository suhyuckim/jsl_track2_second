...
<%
	...
	ArrayList<Event_M_DTO> dtos = dao.getEventApplyList_m(event_no); 
	int status_count = dao.getStatusCount(event_no);
				...
				<table class="table">
					...
					<thead>
						<tr>							
							...
							<th scope="col">응모 번호</th>
							<th scope="col">제목</th>
							<th scope="col">응모자</th>
							<th scope="col">응모일</th>
							<th scope="col">당첨여부</th>												
							<th scope="col">당첨체크</th>
						</tr>
					</thead>
...
%>
						...
						<tr>									
							...
							<td><%=dtos.get(k).getEvent_m_no()%></td>				
						...
						<%if(status_count == 0){ %>
							<td class="yellow"><span>추첨전</span></td>
						<%}else{%>
							...
							<%}%>
						<%}%>				
							<td><input type="checkbox" value="<%=dtos.get(k).getReg_id()%>" name="t_reg_id"></td>
						</tr>
<%
					v_count = v_count + 1;
...
%>
			...
			<div class="page-number">
				...
				<%if(sessionId.equals("manager")){%>
					<input class="btn-write" type="button" onClick="javascript:statuscheck()" value=당첨></a>
				<%}%>
			</div>
		</div>
	</form>
...
<script>
	...
	function statuscheck(){
		var yn = confirm("이벤트 당첨여부를 변경하겠습니까?");
		if(yn == true){
			var fm = document.event;
			fm.action = "event_s_update.jsp";
			fm.method = "post";
			fm.submit();
		}
	}
